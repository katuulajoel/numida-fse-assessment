...
describe('loanUtils', () => {
  describe('calculateLoanStatus', () => {
    ...
    const nearPastDateStr = nearPastDate.toISOString().split('T')[0];
    const farPastDateStr = farPastDate.toISOString().split('T')[0];
    ...
    it('should return On Time when payment is within 5 days of due date', () => {
      expect(calculateLoanStatus(dueDateStr, nearPastDateStr)).toBe('On Time');
    });

    it('should return Late when payment is more than 5 days but less than 30 days late', () => {
      expect(calculateLoanStatus(dueDateStr, pastDateStr)).toBe('Late');
    });

    it('should return Defaulted when payment is more than 30 days late', () => {
      expect(calculateLoanStatus(dueDateStr, farPastDateStr)).toBe('Defaulted');
    });
